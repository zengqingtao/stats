  ...
  <d2-container>
    ...
    <div class="header">
      <div class="statisticsTime">
        ...
        <el-button class="exportdetails" type="primary" @click="exportData">提现明细导出</el-button>
      </div>
    </div>
    <!-- 表格部分 此处自定义的和分页的还没添加-->
    <div>
      ...
      getTransfersList({
        ...
        .then(res => {
          if (res.list.length == 0) {
            ...
            // 把数据转化为数字类型，否则导出后无法累加运算
            for (var i = 0; i < res.list.length; i++) {
              res.list[i].total_fee = Number(res.list[i].total_fee);
            }
            this.$export
              .excel({
